...
teardown_treehouse_return() {
  ...
  local -a th_env=()
  [ -z "$root" ] || th_env=(env "TREEHOUSE_ROOT=$root")
...
cleanup_firstmate_home_children() {
  local home=$1 sub_state child_meta child_id child_t child_wt child_proj child_kind child_home child_backend child_orca_worktree_id child_return_rc child_busy_gen child_owner_rc child_treehouse_root
  sub_state="$home/state"
  [ -d "$sub_state" ] || return 0
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
